...
public class ASCIISerialDataSourceVO<T extends ASCIISerialDataSourceVO<?>>
	...
	protected void addPropertyChangesImpl(List<LocalizableMessage> list, T from) {
		AuditEventType.maybeAddPeriodChangeMessage(list,
				"dsEdit.dnp3.rbePeriod", from.updatePeriodType,
				from.updatePeriods, updatePeriodType, updatePeriods);

		AuditEventType.maybeAddPropertyChangeMessage(list,
				"dsEdit.asciiSerial.commPortId", from.commPortId, commPortId);
		AuditEventType.maybeAddPropertyChangeMessage(list,
				"dsEdit.asciiSerial.baudRate", from.baudRate, baudRate);
		AuditEventType.maybeAddPropertyChangeMessage(list,
				"dsEdit.asciiSerial.dataBits", from.dataBits, dataBits);
		AuditEventType.maybeAddPropertyChangeMessage(list,
				"dsEdit.asciiSerial.stopBits", from.stopBits, stopBits);
		AuditEventType.maybeAddPropertyChangeMessage(list,
				"dsEdit.asciiSerial.parity", from.parity, parity);
		AuditEventType.maybeAddPropertyChangeMessage(list,
				"dsEdit.asciiSerial.timeout", from.timeout, timeout);
		AuditEventType.maybeAddPropertyChangeMessage(list,
				"dsEdit.asciiSerial.retries", from.retries, retries);

		AuditEventType.maybeAddPropertyChangeMessage(list,
				"dsEdit.asciiSerial.stopMode", from.stopMode, stopMode);
		AuditEventType.maybeAddPropertyChangeMessage(list,
				"dsEdit.asciiSerial.nChar", from.nChar, nChar);
		AuditEventType.maybeAddPropertyChangeMessage(list,
				"dsEdit.asciiSerial.charStopMode", from.charStopMode,
				charStopMode);
		AuditEventType.maybeAddPropertyChangeMessage(list,
				"dsEdit.asciiSerial.charX", from.charX, charX);
		AuditEventType.maybeAddPropertyChangeMessage(list,
				"dsEdit.asciiSerial.hexValue", from.hexValue, hexValue);
		AuditEventType
				.maybeAddPropertyChangeMessage(list,
						"dsEdit.asciiSerial.stopTimeout", from.stopTimeout,
						stopTimeout);
		AuditEventType.maybeAddPropertyChangeMessage(list,
				"dsEdit.asciiSerial.initString", from.initString, initString);
		AuditEventType.maybeAddPropertyChangeMessage(list,
				"dsEdit.asciiSerial.bufferSize", from.bufferSize, bufferSize);
