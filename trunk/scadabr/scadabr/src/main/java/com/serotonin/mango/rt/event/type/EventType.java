...
abstract public class EventType implements JsonSerializable {
    ...
    public interface EventSources {
         ...
         * detectors are stored in a single table, so that the id of the
         * detector is a unique identifier for the type. Thus, the detector's id
         * can be (and is) used as the event type id.
         */
        int DATA_POINT = 1;

        /**
         * Data sources raise events internally for their own reasons (for
         * example no response from the external system) or if a point locator
         * failed. Data source error types are enumerated in the data sources
         * themselves. So, the unique identifier of a data source event type is
         * the combination of the the data source id and the data source error
         * type.
         */
        int DATA_SOURCE = 3;

        /**
         * The system itself is also, of course, a producer of events (for
         * example low disk space). The types of system events are enumerated in
         * the SystemEvents class. The system event type is the unique
         * identifier for system events.
         */
        int SYSTEM = 4;

        /**
         * Compound detector event types have their unique identifiers generated
         * by the database. These detectors listen to point event detectors and
         * scheduled events and raise events according to their configured
         * logical statement.
         */
        int COMPOUND = 5;
         ...
         * general publishing failures or failures in individual points. Error
         * types are enumerated in the publishers themselves. So, the unique
         * identifier of a publisher event type is the combination of the
         * publisher id and the publisher error type.
         */
        int PUBLISHER = 7;

        /**
         * Audit events are created when a user makes a change that needs to be
         * acknowledged by other users. Such changes include modifications to
         * point event detectors, scheduled events, compound events, data
         * sources, data points, and point links.
         */
        int AUDIT = 8;

        /**
         * Maintenance events are created when maintenance mode becomes active.
         * See MaintenanceVO for more information.
         */
        int MAINTENANCE = 9;
    }
    ...
    /**
     * This interface defines all of the possible actions that can occur if an
     * event is raised for which type there already exists an active event.
     *
     * @author Matthew Lohbihler
     */
    public interface DuplicateHandling {
        ...
        /**
         * Duplicates are not allowed. This should be the case for all event
         * types where there is an automatic return to normal.
         */
        int DO_NOT_ALLOW = 1;

        /**
         * Duplicates are ignored. This should be the case where the initial
         * occurrence of an event is really the only thing of interest to a
         * user. For example, the initial error in a data source is usually what
         * is most useful in diagnosing a problem.
         */
        int IGNORE = 2;

        /**
         * Duplicates are ignored only if their message is the same as the
         * existing.
         */
        int IGNORE_SAME_MESSAGE = 3;

        /**
         * Duplicates are allowed. The change detector uses this so that user's
         * can acknowledge every change the point experiences.
         */
        int ALLOW = 4;
    }
     ...
     * Convenience method that keeps us from having to cast.
     *
     * @return -1 here, but the data source implementation will return the data
     * source id.
     */
     ...
     * Convenience method that keeps us from having to cast.
     *
     * @return -1 here, but the data point implementation will return the data
     * point id.
     */
     ...
     * Convenience method that keeps us from having to cast.
     *
     * @return -1 here, but the schedule implementation will return the schedule
     * id.
     */
     ...
     * Convenience method that keeps us from having to cast.
     *
     * @return -1 here, but the compound detector event type will return the
     ...
     * Convenience method that keeps us from having to cast.
     *
     * @return -1 here, but the publisher implementation will return the
     * publisher id.
     */
