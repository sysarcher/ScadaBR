...
public class EmailWorkItem implements WorkItem {
    ...
    public void execute() {
        try {
            if (fromAddress == null) {
                String addr = systemSettingsDao.getValue(SysProperties.EMAIL_FROM_ADDRESS);
                String pretty = systemSettingsDao.getValue(SysProperties.EMAIL_FROM_NAME);
                fromAddress = new InternetAddress(addr, pretty);
            }

            EmailSender emailSender = new EmailSender(systemSettingsDao.getValue(SysProperties.EMAIL_SMTP_HOST),
                    systemSettingsDao.getIntValue(SysProperties.EMAIL_SMTP_PORT),
                    systemSettingsDao.getBooleanValue(SysProperties.EMAIL_AUTHORIZATION),
                    systemSettingsDao.getValue(SysProperties.EMAIL_SMTP_USERNAME),
                    systemSettingsDao.getValue(SysProperties.EMAIL_SMTP_PASSWORD),
                    systemSettingsDao.getBooleanValue(SysProperties.EMAIL_TLS));
            ...
            eventManager.raiseEvent(new SystemEventType(SystemEventType.TYPE_EMAIL_SEND_FAILURE),
