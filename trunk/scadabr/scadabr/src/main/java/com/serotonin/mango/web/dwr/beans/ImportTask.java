    ...
    protected void runImpl() {
        try {
            ...
            if (pointValuesIndex < pointValues.size()) {
                importPointValues(pointValues.get(pointValuesIndex++).toJsonObject());
                return;
            }

            if (systemSettingsIndex < systemSettings.size()) {
                importSystemSettings(systemSettings.get(systemSettingsIndex++).toJsonObject());
                return;
            }
            ...
            if (vo != null) {
                try {
                    ...
                    if (voResponse.getHasMessages()) // Too bad. Copy the errors into the actual response.
                    {
                        ...
                        runtimeManager.saveDataPoint(vo);
        ...
        if (StringUtils.isEmpty(xid)) {
            ...
            try {
                ...
                if (voResponse.getHasMessages()) // Too bad. Copy the errors into the actual response.
                {
                    ...
                    // Sweet. Save it.
                    boolean isnew = vo.isNew();
                    runtimeManager.saveScheduledEvent(vo);
        ...
        if (StringUtils.isEmpty(xid)) {
            ...
            try {
                ...
                if (voResponse.getHasMessages()) // Too bad. Copy the errors into the actual response.
                {
                    ...
                    // Sweet. Save it.
                    boolean isnew = vo.isNew();
                    runtimeManager.saveCompoundEventDetector(vo);
            ...
            if (vo != null) {
                try {
                    ...
                    if (voResponse.getHasMessages()) // Too bad. Copy the errors into the actual response.
                    {
                        ...
                        // Sweet. Save it.
                        boolean isnew = vo.isNew();
                        runtimeManager.savePublisher(vo);
        ...
        if (StringUtils.isEmpty(xid)) {
            ...
            try {
                ...
                if (voResponse.getHasMessages()) {
                    ...
                    // Sweet. Save it.
                    boolean isnew = vo.isNew();
                    runtimeManager.saveMaintenanceEvent(vo);
    ...
    private void importPointValues(JsonObject json) {
        ...
        if (dp == null) {
            ...
            PointValueTime pointValue = new PointValueTime(
                    MangoValue.stringToValue(value, dp.getPointLocator().getMangoDataType()), time);
