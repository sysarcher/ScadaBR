...
public final class APIUtils {
    ...
    public static void validateConfigureDataSourceParams(
        ...
        if (params.getType() == DataSourceType.MODBUS_SERIAL) {
            try {
                ModbusSerialConfig config = (ModbusSerialConfig) params.getDataSource();
            } catch (Exception e) {
                error = new APIError();
                error.setCode(ErrorCode.INVALID_PARAMETER);
                error.setDescription(APIConstants.INVALID_PARAMETER
    ...
    public static void validateConfigureDataPointParams(
        ...
        if (params.getType() == DataSourceType.MODBUS_IP
            ...
            try {
                ModbusPointConfig config = (ModbusPointConfig) params.getDataPoint();
            } catch (Exception e) {
                error = new APIError();
                error.setCode(ErrorCode.INVALID_PARAMETER);
                error.setDescription(APIConstants.INVALID_PARAMETER
        ...
        switch (sE.getScheduleType()) {
            case 1:
                config += String.format(
                        "[HOURLY - (active: %d:%d)(inactive: %d:%d)]", sE.getActiveMinute(), sE.getActiveSecond(), sE.getInactiveMinute(), sE.getInactiveSecond());
                break;
            case 2:
                config += String.format(
                        "[DAILY - (active: %d:%d:%d)(inactive: %d:%d:%d)]", sE.getActiveHour(), sE.getActiveMinute(), sE.getActiveSecond(), sE.getInactiveHour(), sE.getInactiveMinute(), sE.getInactiveSecond());
                break;
            case 3: // weekly
                config += String.format(
            ...
            case 5: // yearly
                ...
                config += String.format(
                        ...
                        sE.getActiveMonth(), sE.getActiveDay(), sE.getActiveHour(), sE.getActiveMinute(), sE.getActiveSecond(), sE.getInactiveMonth(),
                        sE.getInactiveDay(), sE.getInactiveHour(), sE.getInactiveMinute(), sE.getInactiveSecond());
                break;
            case 6: // once
                config += String.format(
                        "[ONCE- (active: (year: %d month: %d day: %d - %d:%d:%d))(inactive:(year: %d month: %d day: %d - %d:%d:%d))]",
                        sE.getActiveYear(), sE.getActiveMonth(), sE.getActiveDay(), sE.getActiveHour(), sE.getActiveMinute(), sE.getActiveSecond(),
                        sE.getInactiveYear(), sE.getInactiveMonth(), sE.getInactiveDay(), sE.getInactiveHour(), sE.getInactiveMinute(), sE.getInactiveSecond());
                break;
            case 7: // cron
                config += String.format("[CRON - (active: %s)(inactive: %s)]", sE.getActiveCron(), sE.getInactiveCron());
                break;
    ...
    public static String toCondition(PointEventDetectorVO pointEvent) {
        ...
        DataPointVO dp = new DataPointDao().getDataPoint(new DataPointDao().getDataPointIdFromDetectorId(pointEvent.getId()));
        ...
        switch (pointEvent.getDetectorType()) {
            ...
            case 4: // state multistate
                ...
                break;
            case 5:
                config += String.format("CHANGE (duration: %d)]", pointEvent.getDuration());
                break;
            case 6:
                config += String.format(
                        "STATE_CHANGE_COUNT (count: %d)(duration: %d)]", pointEvent.getChangeCount(), pointEvent.getDuration());
                break;
            case 7:
                config += String.format("NO_CHANGE (duration: %d)]", pointEvent.getDuration());
                break;
            case 8:
                config += String.format("NO_UPDATE (duration: %d)]", pointEvent.getDuration());
                break;
            case 9:
                config += String.format(
                        "ALPHANUMERIC_STATE (state: \"%s\")(duration: %d)]",
                        pointEvent.getAlphanumericState(), pointEvent.getDuration());
                break;
            case 10:
                config += String.format(
                        "POSITIVE_CUSUM (pos limit: %f)(weight:%f)(duration: %d)]",
                        pointEvent.getLimit(), pointEvent.getWeight(), pointEvent.getDuration());
                break;
            case 11:
                config += String.format(
    ...
    public static EventNotification toEventNotification(
        ...
        event.setAlarmLevel(APIUtils.toAlarmLevel(eventInstance.getAlarmLevel()));
        event.setAckTime(ackTime);
        event.setMessage(APIUtils.toEventMessage(eventInstance.getEventComments()));
        ...
        if (eventInstance.getEventType().getDataPointId() != -1) {
            ...
            int pointDetectorId = eventInstance.getEventType().getReferenceId2();

            List<PointEventDetectorVO> peDetectors = new DataPointDao().getDataPoint(dataPointId).getEventDetectors();
            ...
            event.setEventType(EventType.POINT_CONDITION_EVENT);
            int compoundId = eventInstance.getEventType().getCompoundEventDetectorId();
            event.setAlias(new CompoundEventDetectorDao().getCompoundEventDetector(compoundId).getName());
            ...
            event.setAlias(new ScheduledEventDao().getScheduledEvent(scheduleId).getAlias());
        } else {
            event.setEventType(EventType.SYSTEM_EVENT);
    ...
    public static EventMessage[] toEventMessage(List<EventComment> list) {
        if (list != null) {
