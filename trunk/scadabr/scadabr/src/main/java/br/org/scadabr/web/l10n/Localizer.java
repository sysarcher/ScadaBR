...
public class Localizer implements Serializable {
    ...
    public String localizeI18nKey(String i18nKey, Object... args) {
        ...
        try {
            return new MessageFormat(userSessionContextBean.getBundle().getString(i18nKey)).format(localizedArgs);
        } catch (MissingResourceException e) {
            return logAndGetMessage(i18nKey);
        }
    }
     ...
     * @return the pseudo translated key.
     */
    private String logAndGetMessage(String i18nKey) {
        LOG.log(Level.SEVERE, "Localizer found unknown I18N key {0} for locale: {1}.", new Object[]{i18nKey, userSessionContextBean.getLocale()});
        return String.format("!>>>%s:%s<<<!", userSessionContextBean.getLocale(), i18nKey);
    }

    //TODO set TimeZone ???
    public String localizeTimeStamp(long ts, boolean hideDateOfToday) {
        if (hideDateOfToday && (System.currentTimeMillis() - ts) < 86400000) {
            return DateFormat.getTimeInstance(DateFormat.DEFAULT).format(new Date(ts));
        } else {
            return userSessionContextBean.getDateTimeFormat().format(new Date(ts));
        }
    }

    public String localizeDate(long ts) {
        return userSessionContextBean.getDateFormat().format(new Date(ts));
    }

    public String localizeTime(long ts) {
        return userSessionContextBean.getTimeFormat().format(new Date(ts));
    }
