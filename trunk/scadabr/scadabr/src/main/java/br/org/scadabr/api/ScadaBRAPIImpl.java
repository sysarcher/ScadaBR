...
public class ScadaBRAPIImpl implements br.org.scadabr.api.ScadaBRAPI,
    ...
    private ScadaBRAPIDao dataDao = new MangoDaoImpl(AuthenticationHandler.getUsername());
    @Autowired
     ...
     * version, start up time, etc. <br />
     * <br />
     * Este método tenta retornar um objeto do tipo {@link GetStatusResponse}
     * com dados básicos a respeito do servidor, como versão do software, data
     * de inicialização, etc. formatados em um objeto do tipo
    ...
    public br.org.scadabr.api.da.GetStatusResponse getStatus()
        ...
        try {
            //TODO changed from static ServerStateChecker.getState()
            ss.setServerState(new ServerStateChecker().getState());
     ...
     * "maxReturn" can be settable. <br />
     * <br />
     * Este método tenta retornar o valor de uma ou mais TAGs registradas no
     * sistema. Para ler o valor de uma(s) determinada(s) TAGs basta setar os
     * nomes das TAGs em "itemsPath". Como opção, pode-se definir o valor máximo
     * de retorno "maxReturn".
     *
     * @since 0.2
     ...
     * settable to return or not "returnItemValues". <br />
     * <br />
     * Esse método tenta escrever um ou mais valores em TAGs registradas no
     * sistema. Para escrever em alguma(s) TAG(s) deve-se passar os ItemValues
     * com o seu nome qualificado e seu valor de escrita. Como opção de retorno,
     * pode-se definir se os valores escritos nas TAGs serão retornados
     * "returnItemValues".
     *
     * @since 0.2
     *
    ...
    public WriteStringDataResponse writeStringData(
        ...
        for (ItemStringValue itemValue : parameters.getItemsList()) {
            try {
                ItemStringValue newItemValue = dataDao.writeStringData(itemValue);
                if (parameters.getOptions().isReturnItemValues()) {
                    newItemValuesList.add(newItemValue);
                }
                errors.add(APIConstants.ERROR_OK);
        ...
        ItemStringValue[] ivArray = new ItemStringValue[newItemValuesList.size()];
        ivArray = newItemValuesList.toArray(ivArray);
        APIError[] aeArray = new APIError[errors.size()];
        aeArray = errors.toArray(aeArray);
     ...
     * the return's maximum value is settable. <br />
     * <br />
     * Esse método tenta retornar todas ou apenas algumas TAGs especificadas no
     * itemList. Para retornar determinadas TAGs basta setar o itemList com o
     * caminho desejado. Se nada for definido no itemList todas as TAGs serão
     * retornadas. Pode-se definir o número máximo de retorno na resposta
     *
     * @since 0.2
    ...
    public BrowseTagsResponse browseTags(BrowseTagsParams parameters)
        ...
        try {
            itemInfoList = (ArrayList<ItemInfo>) dataDao.browseTags(parameters.getItemsPath());
            errors.add(APIConstants.ERROR_OK);
        } catch (ScadaBRAPIException e) {
            response.setErrors(new APIError[]{e.getError()});
            rb.setReplyTime(Calendar.getInstance());
     ...
     * date. A maximum value can be defined as a option. <br />
     * <br />
     * Este método tenta coletar os dados de uma determinada TAG. Para isso, é
     * necessário determinar um intervalo de tempo, setando uma data inicial e
     * uma data final. Como opção, um valor máximo de dados retornados pode ser
     * definido.
     *
     * @since 0.2
     *
    ...
    public GetDataHistoryResponse getDataHistory(GetDataHistoryParams parameters)
        ...
        try {
            itemValueList = (ArrayList<ItemValue>) dataDao.getDataHistory(parameters.getItemName(), initialDate,
                    finalDate, maxReturn);
            errors.add(APIConstants.ERROR_OK);
        } catch (ScadaBRAPIException e) {
            response.setErrors(new APIError[]{e.getError()});
     ...
     * that can be settable is the maximum's value return. <br />
     * <br />
     * Este método tenta retornar os alarmes ativos do sistema. Para definir o
     * nível dos alarmes desejados, basta definir o nível de alarme "alarmLevel"
     * (os níveis acima também serão retornados), caso não seja setado retorna
     * todos os alarmes. Também como opção pode ser definido o número máximo
     * "maxReturn" de retornos.
     *
     * @since 0.2
     *
     ...
     * event details will be returned, "returnEventDetails". <br />
     * <br />
     * Este método tenta reconhecer os alarmes ativos do sistema. Para
     * reconhecer um alame específico é passada a ID do evento "eventsIds" que
     * quer ser reconhecida. Como outra opção, pode ser definida se os detalhes
     * dos eventos reconhecidos serão retornados, "returnEventDetails".
     *
     * @since 0.2
     *
     ...
     * settable is the maximum's value return. <br />
     * <br />
     * Este método tenta retornar o histórico das notificações de alarmes já
     * reconhecidos. Para isso, é necessário determinar um intervalo de tempo,
     * setando uma data inicial e uma data final. Para definir o nível dos
     * alarmes desejados, basta setar o nível de alarme "alarmLevel" (os níveis
     * acima também serão retornados), caso não seja setado retorna todos os
     * alarmes. Também como opção pode ser definido o número máximo "maxReturn"
     * de retornos.
     *
     * @since 0.2
     *
     ...
     * options of each event just set "returnEventsConfig". <br />
     * <br />
     * Este método tenta retornar as definições de eventos do sistema. Para
     * retornar determinado tipo basta setar o tipo de evento "eventType" para o
     * tipo desejado. Se não for setado algum tipo de evento (null) será
     * retornado todos. Como outra opção, para retornar as opções específicas de
     * cada tipo de evento basta definir "returnEventsConfig".
     *
     * @since 0.2
     *
     ...
     * on it. <br />
     * <br />
     * Este método tenta anotar uma mensagem em um determinado evento. Para
     * anotar uma mensagem em um evento basta passar a ID do evento e também uma
     * mensagem a ser anexada no evento.
     *
     * @since 0.2
     *
    ...
    public br.org.scadabr.api.config.ConfigureDataSourceResponse configureDataSource(
        ...
        int id = 0;
        try {
            id = dataDao.configureDataSource(parameters.getType(), parameters.getDataSource());
            response.setErrors(new APIError[]{new APIError(ErrorCode.OK,
                        "No Problem Found")});
