...
//TODO apl what is this for a class???
@Deprecated
public class MangoDaoImpl implements ScadaBRAPIDao {
    ...
    private boolean isValidDataPoint(DataPointVO dataPointVO) {
        return !(dataPointVO.isNew() || (!dataPointVO.isEnabled() && !dataPointVO.isNew())) && permissions.hasDataPointReadPermission(user,
                dataPointVO);
    }

    private ItemValue toItemValue(DataPointVO dataPointVO) {
        PointValueTime pvt = runtimeManager.getDataPoint(dataPointVO.getId()).getPointValue();
    ...
    public ItemValue writeData(ItemValue itemValue) throws ScadaBRAPIException {
        ...
        PointHierarchy localPointHierarchy = dataPointDao.getPointHierarchy();
        ...
        if (dp != null) {
            ...
            MangoValue value = MangoValue.stringToValue(itemValue.getValue().toString(), dp.getPointLocator().getMangoDataType());
            runtimeManager.setDataPointValue(dp.getId(), value, null);
            flag = true;
            newItemValue.setValue(itemValue.getValue());
            newItemValue.setItemName(dp.getName());

        } else {
            List<DataPointVO> listDPVO = dataPointDao.getDataPoints(null,
                    false);
            for (DataPointVO dataPointVO : listDPVO) {
                String completeName = APIUtils.getCompletePath(
                        dataPointVO.getPointFolderId(), localPointHierarchy)
                        + dataPointVO.getName();
                if (itemValue.getItemName().equals(completeName)) {
                    ...
                    MangoValue value = MangoValue.stringToValue(itemValue.getValue().toString(), dataPointVO.getPointLocator().getMangoDataType());
                    runtimeManager.setDataPointValue(
    ...
    private void checkValidWriteCommand(DataPointVO dataPointVO,
        ...
        PointValueTime pvt = pointValueDao.getLatestPointValue(dataPointVO.getId());
    ...
    public ItemStringValue writeStringData(ItemStringValue itemValue)
        ...
        int dataPointId = Common.ctx.getDataPointByName(itemValue.getItemName());
        DataPointRT dpRT = runtimeManager.getDataPoint(
                dataPointId);
        DataPointVO dp = null;
        if (dpRT != null) {
            dp = dpRT.getVO();
        ...
        if (dp != null) {
            ...
            runtimeManager.setDataPointValue(dp.getId(), value,
                    null);
            ...
            for (DataPointVO dataPointVO : listDPVO) {
                ...
                if (itemValue.getItemName().equals(completeName)) {
                    ...
                    MangoValue value = MangoValue.stringToValue(itemValue.getValue(), dataPointVO.getPointLocator().getMangoDataType());
                    runtimeManager.setDataPointValue(
                            dataPointVO.getId(), value, null);
    ...
    public List<ItemInfo> browseTags(String itemPath)
        ...
        List<DataPointVO> ldpvo = dataPointDao.getDataPoints(null, false);
        PointHierarchy localPointHierarchy = dataPointDao.getPointHierarchy();
            ...
            if (APIUtils.isInPath(completeName, itemPath)
                ...
                if (dataPointVO.isEnabled()) {
                    ...
                    if (permissions.hasDataPointReadPermission(user,
                        ...
                        itemInfo.setDataType(APIUtils.toDataType(dataPointVO.getPointLocator().getMangoDataType()));
    ...
    public List<ItemValue> getDataHistory(String itemQName,
        ...
        for (DataPointVO dataPointVO : listDPVO) {
            ...
            if (itemQName.equals(completeName)) {
                ...
                List<PointValueTime> lpvt = pointValueDao.getPointValuesBetween(dataPointVO.getId(),
    ...
    public List<Object> getDataSources(DataSourceType dataSourceType)
        ...
        List<DataSourceVO<?>> allDataSources = dataSourceDao.getDataSources();
    ...
    @Override
    public List<Object> getDataPoints(DataSourceVO<?> dataSourceVo)
            throws ScadaBRAPIException {
        checkSupportedDataPoints(dataSourceVo);
        checkUser();
        ...
        List<DataPointVO> mangoPoints = dataPointDao.getDataPoints(dataSourceVo, null);
    ...
    private void checkSupportedDataPoints(DataSourceVO<?> dataSourceVo)
            throws ScadaBRAPIException {
        if (dataSourceVo.getType() != DataSourceRegistry.MODBUS_IP
                && dataSourceVo.getType() != DataSourceRegistry.MODBUS_SERIAL) {
            throw new ScadaBRAPIException(new APIError(ErrorCode.INVALID_ID,
    ...
    @Override
    public int configureDataPoint(DataSourceVO<?> dataSourceVo,
        ...
        mangoPoint.setDataSourceId(dataSourceVo.getId());
        ...
        runtimeManager.saveDataPoint(mangoPoint);
