...
public class Upgrade0_7_0 extends DBUpgrade {
    ...
    private final static Logger LOG = LoggerFactory.getLogger(Upgrade0_7_0.class);
    ...
    private void insertDataSource(final DataSourceVO<?> vo) {
        getSimpleJdbcTemplate().update("insert into dataSources (id, name, dataSourceType, data) values (?,?,?,?)",
    ...
    private void setRelationalData(DataPointVO dp) {
        if (dp.getDataSourceTypeId() == DataSourceRegistry.VIRTUAL.mangoId) {
            ...
            if (changeType == 4 || changeType == 8) {
                ...
                getJdbcTemplate().query("select multistateValue from locatorVrtValues where dataPointId=? order by valueOrder",
        ...
        if (dp.getTextRenderer() instanceof MultistateRenderer) {
            ...
            // Extract the values from the database.
            getJdbcTemplate().query("select multistateKey, multistateValue, multistateColour from dataPointMultistateValues "
            ...
            // Extract the values from the database.
            getJdbcTemplate().query("select rangeFrom, rangeTo, rangeValue, rangeColour from dataPointRangeValues "
                    + "where dataPointId=? order by valueOrder", new Object[]{dp.getId()}, new RowCallbackHandler() {
    ...
    private void insertDataPoint(final DataPointVO vo) {
        getJdbcTemplate().update("insert into dataPoints (id, dataSourceId, data) values (?,?,?)", new PreparedStatementSetter() {
